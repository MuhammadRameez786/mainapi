...
exports.getAllNfts = catchAsync( async (req, res) => {
  ...
  try {
    ...
    const features = new APIFeatures(
      NFT.find({ collectionName: collectionName }),
      req.query
    )
      .filter()
      .sort()
...
exports.getTop5Creators = catchAsync(async (req, res) => {
  // create a custom property on the request object to hold SSE clients
  req.sseClients = [];
  // set up a SSE stream to push updates to the client
  res.writeHead(200, {
    "Content-Type": "text/event-stream;charset=utf-8",
    "Cache-Control": "no-cache",
    "Connection": "keep-alive"
  ...
  // send an initial "ping" message to the client to test the connection
  res.write("event: ping\ndata: \n\n");
...
exports.aliasTopNFTs = catchAsync(async (req, res, next) => {
  ...
  req.sseClients.push(res);
  try {
    const top5NFTs = await NFT.find().sort("-price").limit(5).select("name price seller image createAt");
    // push updates to all connected SSE clients
    req.sseClients.forEach((client) => {
      client.write(`event: update\ndata: ${JSON.stringify(top5NFTs)}\n\n`);
    });

    //res.json(result);
    console.log("Results", top5NFTs);
  } catch (err) {
    console.error(err);
    res.status(500).send('Internal Server Error');
  }
